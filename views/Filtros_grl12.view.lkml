view: filtros_grl12 {
  ...
  dimension: tipo_movimiento {
    type: string
    sql:  case when ${TABLE}.TipoMovimiento like '%evol%' then 'Devoluciones' when ${TABLE}.TipoMovimiento in ('No Aplica', 'Otros') then 'Excluir' when ${TABLE}.TipoMovimiento in ('Comisione', 'Comisiones') Then 'Comisiones'  else ${TABLE}.TipoMovimiento end  ;;
  }
