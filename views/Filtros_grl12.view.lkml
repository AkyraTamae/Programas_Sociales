view: filtros_grl12 {
  ...
  dimension: tipo_movimiento {
    type: string
    sql:  case when ${TABLE}.TipoMovimiento like '%evol%' then 'Devoluciones' when ${TABLE}.TipoMovimiento like '%plica%' then 'No Aplica' when ${TABLE}.TipoMovimiento In ('Comisione', 'Comisiones') Then 'Comisiones'  else ${TABLE}.TipoMovimiento end  ;;
  }
  ...
  measure: importe_pesos_sum {
    ...
    sql: case when ${tipo_movimiento} = 'Devoluciones' then -1 * ${TABLE}.ImportePesos else ${TABLE}.ImportePesos end ;;
  }
